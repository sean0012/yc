...
var paramList = [
  "mid" + ":String,1~10", "oid" + ":String,1~40", "price" + ":Number,1~64", "currency" + ":String,3", "buyertel" + ":String,1~20", "timestamp" + ":String,1~20", "mKey" + ":String,1~128"
];
...
var $jINILoader = {
  ...
  waitForJQueryLoad: function() {
    setTimeout(function() {
      if (!$jINILoader.$jINILoadChecker()) {
        $jINILoader.waitForJQueryLoad();
      } else {
        _startupJob();
      }
    }, 100);
  },
  $jINILoadChecker: function() {
    try {
      var win = $jINI(window);
      return true;
    } catch (ex) {
  ...
  submit: function(jsonData, status, jqXHR) {
    ...
    if (!INIStdPay.boolMobile && ((INIStdPay.vPayViewType == "overlay" && !INIStdPay.boolPayRequestCheck) || (!INIStdPay.boolViewParentWindow && !INIStdPay.boolPayRequestCheck))) {
      ...
      if (INIStdPay.boolViewParentWindow || INIStdPay.boolCard || INIStdPay.boolHpp) {
        ...
        setTimeout(function() {
          INIStdPay.$modalDiv.show();
        }, 1000); // iframe submit 할때 깜빡임 안보이게 하기
